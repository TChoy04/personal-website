...
function CoinBar(prop) {
  return (
    <>
            ...
            <div className="coin-imagetext"><img src={prop.coin.image} alt="crypto"></img><p className="coin">{prop.coin.symbol.toUpperCase()}</p></div>
            <p className="price">{prop.coin.current_price.toLocaleString()}</p>
            <p className="daily">{`${prop.coin.price_change_percentage_24h.toFixed(2)}%`}</p>
